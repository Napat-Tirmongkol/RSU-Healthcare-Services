...
`;
                ...
                .then(d => {
                    ...
                    if (borrowsSub) {
                        if (d.borrows > 0) {
                            ...
                            borrowsSub.innerHTML = '<i class="fa-solid fa-circle-exclamation" style="margin-right:3px"></i>????????????????????????????????????';
                        } else {
                            borrowsSub.style.color = '#94a3b8';
                            borrowsSub.textContent = '???????????????????????????????????????????????????????????????';
                        }
                    }
        ...
        function switchIdTab(tab, btn) {
            ...
            if (search) {
                search.value = '';
                idUniversalFilter('');
                search.placeholder = isUsers ? '??????????????? Users...' : (isAdmins ? '??????????????? Admins...' : '??????????????? Staff...');
            }
        }
        ...
        function openAddAdminModal() {
            ...
            document.getElementById('admModalTitle').textContent = '??????????????? System Admin';
        ...
        function openAddStaffModal() {
            ...
            document.getElementById('sfModalTitle').textContent = '???????????????????????????????????????????????? (Staff)';
        ...
        function openEditStaffModal(st) {
            ...
            document.getElementById('sfModalTitle').textContent = '??????????????????????????????????????????????????????????????????';
        ...
        function idOpenView(u) {
            var statusMap = { student: '????????????????????????', staff: '?????????????????????/?????????????????????', teacher: '?????????????????????', other: '?????????????????????????????????' };
            var genderMap = { male: '?????????', female: '????????????', other: '???????????????' };
            var map = [
                ['????????????-?????????????????????', u.full_name],
                ['??????????????????????????????????????????', u.citizen_id],
            ...
            if (u.status === 'other' && u.status_other) {
                map.push(['?????????????????????????????????', u.status_other]);
            }
            map.push(['?????????????????????????????????????????????', u.created_at ? new Date(u.created_at.replace(' ', 'T')).toLocaleString('th-TH', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '???']);
        ...
        (function () {
            ...
            function render() {
                ...
                if (info) info.textContent = total === 0 ? '?????????????????????????????????' : from + '???' + to + ' ????????? ' + total.toLocaleString();
        ...
        function updateMaintenanceUI(project, active) {
            ...
            if (banner) {
                ...
                if (title) title.textContent = allOn ? '??????????????????????????????????????????????????????????????????????????????' : '????????????????????????????????????????????????????????????????????????????????????';
                if (desc) desc.textContent = allOn ? 'User ?????????????????????????????????????????????????????????????????????????????????????????????' : '???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????';
        ...
        function toggleMaintenance(input) {
            ...
            fetch('ajax_maintenance.php', { method: 'POST', body: fd })
                ...
                .then(d => {
                    if (d.ok) {
                        showPortalToast(active ? `${project} ??????????????????????????????????????????` : `${project} ?????????????????????????????????????????????`, active ? 'success' : 'error');
                    } else {
                        input.checked = !active;
                        updateMaintenanceUI(project, !active);
                        showPortalToast('?????????????????????: ' + (d.message || 'Unknown error'), 'error');
                    }
                })
                .catch(() => {
                    input.checked = !active;
                    updateMaintenanceUI(project, !active);
                    showPortalToast('????????????????????????????????????????????????????????????????????????????????????????????????', 'error');
                });
        }
