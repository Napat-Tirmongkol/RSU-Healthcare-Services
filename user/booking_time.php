            ...
            <div class="space-y-3">
                ...
                <?php else: ?>
                    <?php foreach ($timeSlots as $slot): 
                        ...
                        $isFull = $remaining <= 0;
                    ?>
                        <label id="slot-label-<?= $slot['id'] ?>" class="relative block bg-white border <?= $isFull ? 'border-red-200 opacity-60' : 'border-gray-200 cursor-pointer hover:border-[#2e9e63] hover:bg-green-50/50 hover:shadow-sm' ?> rounded-2xl p-4 transition-all duration-300">
                ...
                if(data.status === 'success') {
                    for (const [slotId, remaining] of Object.entries(data.data)) {
                        ...
                        if(remaining <= 0) {
                            ...
                            label.className = "relative block bg-white border border-gray-200 cursor-pointer hover:border-[#2e9e63] hover:bg-green-50/50 hover:shadow-sm rounded-2xl p-4 transition-all duration-300";
