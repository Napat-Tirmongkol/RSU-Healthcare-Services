...
<body class="text-slate-900 pb-32">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (BEAMS_ID) {
                console.log('Initializing Pusher Beams with ID:', BEAMS_ID);
                
                // Check if running on a secure origin
                if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
                    console.error('Pusher Beams requires HTTPS or localhost to work.');
                    return;
                }
                ...
                beamsClient.start()
                    .then(() => {
                        console.log('Beams SDK started. Requesting device interest...');
                        return beamsClient.addDeviceInterest('user-<?= (int)($_SESSION['user_id'] ?? 0) ?>');
                    })
                    .then(() => {
                        console.log('Successfully registered with Pusher Beams!');
                        // Optional: Subscribe to a global interest too
                        return beamsClient.addDeviceInterest('global');
                    })
                    .then(() => console.log('Subscribed to interests.'))
                    .catch(err => {
                        console.error('Pusher Beams Error:', err);
                        if (err.message && err.message.includes('permission')) {
                            alert('กรุณาอนุญาตการแจ้งเตือนใน Browser ของคุณ เพื่อรับข่าวสารจากทางคลินิกครับ');
                        }
                    });
            }
        });
